...
def fave_object(request, content_type_id, object_id, fave_type_slug, success_template_name = 'faves/fave_done.html'):
    ...
    if request.is_ajax():
        context = '{"success": "true", "content_object_id": "%s", "new_href":"%s"}' % (faved_object.id, reverse('unfave_object', kwargs = {'content_type_id':content_type_id,
                                                                                                 'object_id':object_id,
                                                                                                 'fave_type_slug':fave_type_slug
                                                                                                 }))
        return HttpResponse(context, mimetype = "application/json")
    else:
...
def unfave_object(request, content_type_id, object_id, fave_type_slug, success_template_name = 'faves/unfave_done.html'):
    ...
    if request.is_ajax():
        context = '{"success": "true", "content_object_id": "%s", "new_href":"%s"}' % (faved_object.id, reverse('fave_object', kwargs = {'content_type_id':content_type_id,
                                                                                                 'object_id':object_id,
                                                                                                 'fave_type_slug':fave_type_slug
                                                                                                 }))
        return HttpResponse(context, mimetype = "application/json")
