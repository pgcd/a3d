...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for item in orm.UserProfile.objects.all():
            item.timestamp = 0xFFFFFFFF - item.reverse_timestamp
            item.save()

        for item in orm.TagAttach.objects.all():
            item.timestamp = 0xFFFFFFFF - item.reverse_timestamp
            item.save()

        for item in orm.Post.objects.all():
            item.timestamp = 0xFFFFFFFF - item.reverse_timestamp
            item.save()
    ...
    models = {
        ...
        'board.userprofile': {
            ...
            'last_page_time': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2011, 2, 22, 11, 51, 30, 766639)', 'db_index': 'True'}),
